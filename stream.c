...
void stream_writef(Stream self, const char * format, ...){
    ...
    va_start(va, format);
    length = vsnprintf(data,length+1,format,va);
    va_end(va);
    ...
    switch (self->type){
        ...
        case 'i': // STDIO        
            if (self->output_descriptor == -1) break;
            write(self->output_descriptor, data, length) ;
            break;
        case 's': // SOCKET
            if (self->output_descriptor == -1) break;
            send(self->output_descriptor ,data, length,0 );
            break;
        case 'm': // MEMORY
            /* make sure there is enough room for the data */
            stream_alloc(self,length);

            /* Write the data */
            memcpy(self->buffer+self->write_offset, data, length);
